...
import { SepaTransferPaymentInfo } from "./SepaTransferPaymentInfo";

export class SepaTransferDocument extends PainXmlFile {
  constructor(
    private data: {
      messageId: string;
      created: Date;
      ...
      payments: SepaTransferPaymentInfo[];
    }
  ) {
    super();
  }
  ...
  toXmlStructure(
    ...
    if (!configuration.disableValidation) {
      assert(format.type === PainType.Transfer, "Invalid pain format type");
