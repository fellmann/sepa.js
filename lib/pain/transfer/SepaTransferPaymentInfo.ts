...
import { SepaTransferTransaction } from "./SepaTransferTransaction";

export class SepaTransferPaymentInfo implements PainXmlSerializable {
  constructor(
    public data: {
      paymentId: string;
      ...
      transactions: SepaTransferTransaction[];
    }
  ) {}
  ...
  toXmlStructure(
    ...
    return {
      ...
      ReqdExctnDt: format.requestedExecutionDatePlain
        ? this.data.requestedExecutionDate.toISOString().substring(0, 10)
        : {
            Dt: this.data.requestedExecutionDate.toISOString().substring(0, 10),
          },
      Dbtr: {
      ...
      ChrgBr: "SLEV",
      CdtTrfTxInf: this.data.transactions.map((t) =>
